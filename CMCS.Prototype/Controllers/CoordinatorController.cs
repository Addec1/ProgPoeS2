...
namespace CMCS.Prototype.Controllers
{
    public class CoordinatorController : Controller
    {
        ...
        public CoordinatorController(IFileStore store) => _store = store;
        ...
        public IActionResult Verify(Guid id)
        {
            ...
            if (c.Status == ClaimStatus.Submitted) c.Status = ClaimStatus.Verified;
        ...
        public IActionResult Reject(Guid id)
        {
            var c = ClaimStore.Find(id);
            if (c is null) return NotFound();
            if (c.Status is ClaimStatus.Submitted or ClaimStatus.Verified) c.Status = ClaimStatus.Rejected;
